  ...
  fun! s:Doxygen_Hilights()
    ...
    if exists('g:doxygen_codeword_font')
      ...
      if has('gui_running')
        ...
        elseif has('gui_athena') || has('gui_gtk') || &guifont=~'^\(-[^-]\+\)\{14}'
          if &guifont == ''
            let font='font=-b&h-LOG1idatypewriter-medium-r-normal-*-*-140-*-*-m-*-iso8859-1'
          else
          " let font='font='.substitute(&guifont,'^\(-[^-]\+\)\{7}-\([0-9]\+\).*', '-b\&h-LOG1idatypewriter-medium-r-normal-*-*-\2-*-*-m-*-iso8859-1','')
    ...
    if (exists('g:doxygen_enhanced_color') && g:doxygen_enhanced_color) || (exists('g:doxygen_enhanced_colour') && g:doxygen_enhanced_colour)
      if &background=='light'
        ...
        SynLink doxygenComment Comment
      ...
      SynLink doxygenComment Comment
      SynLink doxygenBrief Comment
      SynLink doxygenBody Comment
      SynLink doxygenSpecialTypeOnelineDesc Statement
      SynLink doxygenBOther Macro
      SynLink doxygenParam Macro
      SynLink doxygenParamName Underlined
      SynLink doxygenSpecialOnelineDesc Comment
      SynLink doxygenSpecialHeading Comment
